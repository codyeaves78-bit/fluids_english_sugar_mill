...
def list_functions() -> None:
    """Print every public function in this file: name, inputs, one-line what-it-do.

    Skips private helpers (name starts with _, like _beta and
    _interp_table) - those are just plumbing for the public functions
    below them, not meant to be called on their own.
    """
    this_module = sys.modules[__name__]
    for name, obj in inspect.getmembers(this_module, inspect.isfunction):
        if name.startswith("_") or obj.__module__ != __name__:
            continue
